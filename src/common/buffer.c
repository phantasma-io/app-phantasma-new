...
bool to_hex(const char ch0, const char ch1, uint8_t *out) {
...
bool buffer_read_hex(buffer_t *script_buf, uint8_t *out) {
    uint8_t ch0;
    uint8_t ch1;
    if (!buffer_read_u8(script_buf, &ch0)) {
        return false;
    }
    if (!buffer_read_u8(script_buf, &ch1)) {
        return false;
    }
    return to_hex(ch0, ch1, out);
}
