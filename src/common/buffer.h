...
/**
 * convert two bytes as hex.
 *
 * @param[in]  ch0
 *   Pointer to input char 0.
 * @param[in]  ch1
 *   Pointer to input char 1.
 * @param[out]     value
 *   Pointer to 8-bit unsigned integer output.
 *
 * @return true if success, false otherwise.
 *
 */
bool to_hex(const char ch0, char ch1, uint8_t *out);

/**
 * read two bytes from buffer as hex.
 *
 * @param[in,out]  buffer
 *   Pointer to input buffer struct.
 * @param[out]     value
 ...
 *
 */
bool buffer_read_hex(buffer_t *buffer, uint8_t *out);
